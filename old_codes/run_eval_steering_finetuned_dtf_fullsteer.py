...
from util import obtain_icv_dtf_test, obtain_icv_dtf, add_icv_layers_confidence, add_icv_layers_new, remove_icv_layers, retrieve_examples_new, icv_tightness
...
def evaluate(args):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(test_dl)):
            ...
            # remove indices that has similarities < threshold
            selected_similarities = [s for s in selected_similarities if s > args.threshold]
            ...
            icv = obtain_icv_dtf_test(gt_list, pred_list, hidden_size=384)
            icv = icv[1:]  # keep in sync with your pipeline
            icv = icv.to(device=device, dtype=model.dtype)

            # inject ICV
            add_icv_layers_confidence(model.model, torch.stack([icv], dim=1).cuda() if device == 'cuda' else torch.stack([icv], dim=1), [args.lam])
            # add_icv_layers_new(model.model, torch.stack([icv], dim=1).cuda() if device == 'cuda' else torch.stack([icv], dim=1), [0.01])
