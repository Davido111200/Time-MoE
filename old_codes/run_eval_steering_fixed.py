...
def evaluate(args):
    ...
    if args.pool_number is None:
        SCRATCH_DIR = f"/scratch/s223540177/Time-Moe/cache_data_fixed_full"
    else:
        SCRATCH_DIR = f"/scratch/s223540177/Time-Moe/cache_data_fixed_{args.pool_number}"
    ...
    if os.path.exists(pred_path):
        ...
        with torch.no_grad():
            for idx, (batch) in enumerate(tqdm(train_dl)):
                ...
                preds, labels = cached_model.predict(batch)
                ip_and_preds = torch.cat([batch['inputs'].to("cuda"), preds], dim=1).squeeze(-1)  # [1, D, T1+T2]
                r, hs = get_rep_with_hidden_states(cached_model.model, batch['inputs'], ip_and_preds)
    ...
    if os.path.exists(gt_path):
        ...
        with torch.no_grad():
            for idx, (x, y, x_mark, y_mark) in enumerate(tqdm(train_dl_gt)):
                ...
                r, hs = get_rep_with_hidden_states(cached_model.model, x, ip_and_gt)
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(test_dl)):
            ...
            # Calibrated on your illness run:
            # STD_GATE = 0.0298  # ≈ median std; use 0.027 for stricter/less coverage
            # STD_GATE = 0.0192

            # should_steer = (std_d <= STD_GATE)
            icv = obtain_icv_new(gt_list, pred_list, hidden_shape=384, rank=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pool_number', default=None)
