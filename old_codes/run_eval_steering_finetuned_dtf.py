...
from util import obtain_icv_dtf, add_icv_layers, remove_icv_layers, retrieve_examples_new, cap_each_id
...
def evaluate(args):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(test_dl)):
            ...
            icv = obtain_icv_dtf(gt_list, pred_list, hidden_size=384, whiten=False, normalize_per_layer=False)
            ...
            if 'etth2' in args.data_path.lower() or 'etth1' in args.data_path.lower():
                ret_metric = {}
                for metric in metric_list:
    ...
    print(f'ICV applied times: {icv_counter}')
