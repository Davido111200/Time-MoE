...
def evaluate(args):
    ...
    # output_log = f"/home/s223540177/Time-MoE/metric_results/timerxl/{args.data}_zeroshot.txt"
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(test_dl)):
            preds = model.generate(batch["inputs"], max_new_tokens=args.pred_len)
            labels = batch['labels']

            mse_base_step = torch.mean((preds - labels) ** 2).item()
            print(f'Step {idx} - MSE: {mse_base_step}')

            if math.isnan(mse_base_step):
    ...
    print(f'{rank} - NaN batch count: {nan_count}')
